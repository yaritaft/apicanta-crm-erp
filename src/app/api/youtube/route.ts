...
type Miniaturas = Record<string, { url?: string } | undefined>;
...
const numero = (s?: string) => (s !== undefined && s !== "" && Number.isFinite(Number(s)) ? Number(s) : undefined);

const mejor = (m: Miniaturas | undefined, orden: string[]) =>
  orden.map((k) => m?.[k]?.url).find(Boolean);

async function pedirApi<T>(
  recurso: "videos" | "channels", params: Record<string, string>, clave: string,
...
function explicar(status: number, motivos: string, mensaje: string): string {
